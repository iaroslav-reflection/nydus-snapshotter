...
type ConfigState struct {
	...
	ID        string
	ProcessID int
	// Start time of process `ProcessID` after system boot, in clock ticks
	// (field 22 of `/proc/<pid>/stat`). PID and start time together identify
	// a process instance; a bare PID may have been recycled by an unrelated
	// process. Zero for records persisted by older snapshotter versions.
...
// RecordProcess remembers the identity of the just started daemon process:
// its PID together with the process start time, so that later signaling can
// tell the process apart from an unrelated one that recycled the PID.
